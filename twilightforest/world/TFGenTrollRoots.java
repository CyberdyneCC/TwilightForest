...
public class TFGenTrollRoots extends TFGenerator {
    ...
    public boolean func_76484_a(World par1World, Random par2Random, int x, int y, int z) {
        ...
        for (int copyZ = z; y > 5; --y) {
            if (par1World.func_147437_c(x, y, z) && BlockTFTrollRoot.canPlaceRootBelow(par1World, x, y + 1, z) && par2Random.nextInt(6) > 0) {
                if (par2Random.nextInt(10) == 0) {
                    par1World.func_147449_b(x, y, z, TFBlocks.unripeTrollBer);
                } else {
                    par1World.func_147449_b(x, y, z, TFBlocks.trollVidr);
                }
            } else {
